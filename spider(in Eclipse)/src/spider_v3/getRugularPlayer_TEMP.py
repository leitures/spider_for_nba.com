...
class getRegularPlayer:
    ...
    def savePlayerInfo(self,s,i):
        ...
        PTS_PAINT = s["resultSets"][1]["rowSet"][i][4]
        PTS_2ND_CHANCE =s["resultSets"][1]["rowSet"][i][5]
        PTS_FB =s["resultSets"][1]["rowSet"][i][6]
        LARGEST_LEAD = s["resultSets"][1]["rowSet"][i][7]
        LEAD_CHANGES = s["resultSets"][1]["rowSet"][i][8]
        TIMES_TIED = s["resultSets"][1]["rowSet"][i][9]

           
        str1 ="'"+GAME_DATE+"','"+GAME_ID+"','"+str(TEAM_ID)+"','"+TEAM_ABBREVIATION+"','"+TEAM_WINS_LOSSES+"','"+str(QTR1)+"','"+str(QTR2)+"','"+str(QTR3)+"','"+str(QTR4)+"','"+str(OT1)+"','"+str(OT2)+"','"+str(OT3)+"','"+str(OT4)+"','"+str(OT5)+"','"+str(OT6)+"','"+str(OT7)+"','"+str(PTS)+"','"+str(PTS_PAINT)+"','"+str(PTS_2ND_CHANCE)+"','"+str(PTS_FB)+"','"+str(LARGEST_LEAD)+"','"+str(LEAD_CHANGES)+"','"+str(TIMES_TIED)+"'"                                         
        #打开数据库链接
        ...
        #使用cursor()方法获取操作游标
        cursor = db.cursor()
        sql = "INSERT INTO TEAMINFO(GAME_DATE,GAME_ID,TEAM_ID,TEAM_ABBREVIATION,TEAM_WINS_LOSSES,QTR1,QTR2,QTR3,QTR4,OT1,OT2,OT3,OT4,OT5,OT6,OT7,PTS,PTS_PAINT,PTS_2ND_CHANCE,PTS_FB,LARGEST_LEAD,LEAD_CHANGES,TIMES_TIED) VALUES ("+str1+")"
