...
#PLUS_MINUS CHAR(10)
        ...
        str1 ="'"+GAME_ID+"','"+str(TEAM_ID)+"','"+TEAM_ABBREVIATION+"','"+TEAM_CITY+"','"+str(PLAYER_ID)+"','"+PLAYER_NAME+"','"+START_POSITION+"','"+str(COMMENT)+"','"+str(MIN)+"','"+str(FGM)+"','"+str(FGA)+"','"+str(FG_PCT)+"','"+str(FG3M)+"','"+str(FG3A)+"','"+str(FG3_PCT)+"','"+str(FTM)+"','"+str(FTA)+"','"+str(FT_PCT)+"','"+str(OREB)+"','"+str(DREB)+"','"+str(REB)+"','"+str(AST)+"','"+str(STL)+"','"+str(BLK)+"','"+str(TOV)+"','"+str(PF)+"','"+str(PTS)+"','"+str(PLUS_MINUS)+"'"                                         
        #打开数据库链接
        ...
        #使用cursor()方法获取操作游标
        cursor = db.cursor()
        sql = "INSERT INTO PLAYOFFPLAYER(GAME_ID,TEAM_ID,TEAM_ABBREVIATION,TEAM_CITY,PLAYER_ID,PLAYER_NAME,START_POSITION,COMMENT,MIN,FGM,FGA,FG_PCT,FG3M,FG3A,FG3_PCT,FTM,FTA,FT_PCT,OREB,DREB,REB,AST,STL,BLK,TOV,PF,PTS,PLUS_MINUS) VALUES ("+str1+")"
        try:
            # 执行sql语句
            cursor.execute(sql)
        # 提交到数据库执行
...
getPLAYOFFPlayer = getPlayoffPlayer()
"""
